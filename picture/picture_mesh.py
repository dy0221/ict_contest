...
def face_mesh(image_loction: str) -> list:
  #check picture is available
  try:
    image = cv2.imread(image_loction)
  except:
    return  
  #setup thresshold
...
if __name__ =='__main__':
  ...
  from picture_package import face_draw
  from picture_package import face_connections
  from picture_package import face_triangle_connection
  while True:
    image_location = '/home/dy/opencv_study_project/resize2_image1.jpeg'
    image = face_mesh(image_location)
    # cv2.imshow('11', image)
    # for tuple in face_connections.LEFTEYEAREA:
    #   f , s, t = tuple
    #   fs ,ss   = image[f]
    # print('{0},{1}'.format(fs,ss))
    print(image)
  ...
  from picture.picture_package import face_draw
  from picture.picture_package import face_connections
  from picture.picture_package import face_triangle_connection
